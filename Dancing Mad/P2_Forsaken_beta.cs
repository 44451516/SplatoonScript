...
public class P2_Forsaken_beta : SplatoonScript<P2_Forsaken_beta.Config>
{
    ...
    {
        En =
            "Forsaken beta 是 P2 Missing 的动态辅助。设置项只保留全局优先级、可选搭档、波次分组和精简职责位置表。",
        Jp =
    ...
    {
        En =
            "设置全局优先级、可选显式搭档，以及这些搭档如何分成 1/2/3/8 组与 4/5/6/7 组。没有显式搭档时，脚本会使用优先级搭档 1+3、2+4、5+7、6+8。",
        Jp =
    ...
    {
        En =
            "当 4 组显式搭档都能匹配当前小队时，会优先使用显式搭档而不是优先级默认搭档。若采用头顶分摊所在搭档整组处理 1/2/3/8 波的打法，请保持搭档分割关闭。",
        Jp =
    ...
    {
        En =
            "第 3 波后，前半组的圆形/扇形玩家可以给自己标点供第 8 波使用。实战执行命令需要手动开启；副本录像只会记录将要执行的命令。",
        Jp =
    ...
    {
        En =
            "关闭：只在本地打印命令，不实际发送。开启：仅在非副本录像回放中发送已配置命令。",
        Jp =
    ...
    {
        En = "实战标记命令会在此范围内随机延迟后发送。录像回放和空跑日志仍会立即执行。",
    ...
    {
        En =
            "开启后，第 8 波圆形/扇形排名可以从所选标点类型解析。关闭时，沿用原本的 Debuff 与优先级逻辑。",
        Jp =
    ...
    {
        En =
            "开启后，第 8 波会将标点 1 视为排名 1、标点 2 视为排名 2。适用于第 3 波后自标代表左 1 / 右 2 的打法。",
        Jp =
    ...
    {
        En =
            "关闭：含 1 名头顶分摊玩家的搭档整组处理 1/2/3/8 波，不含头顶分摊的搭档整组处理 4/5/6/7 波。开启：头顶分摊搭档中只有头顶分摊本人处理 1/2/3/8 波，其搭档处理 4/5/6/7 波。不含头顶分摊的搭档按全局优先级拆分，高优先级去 1/2/3/8，低优先级去 4/5/6/7。",
        Jp =
    ...
    {
        En =
            "开启后，第 2 波及以后头顶分摊左右分配会依据玩家上一次 Debuff：上次扇形 → 左分摊，上次圆形 → 右分摊。关闭时使用分组列表顺序。",
        Jp =
    ...
    {
        En =
            "开启后，若当前同标点玩家上一轮处理的是同一座塔，离场地中心更近的人保留较前排名，较远的人获得较后排名。每个排名的落点仍由位置表决定。",
        Jp =
    ...
    {
        En =
            "控制第 1 波头顶分摊左右分配。搭档 Debuff 会读取头顶分摊玩家搭档的标记；优先级顺序使用当前全局优先级；职责侧会将全局优先级 1-4 分到左头顶分摊位，5-8 分到右头顶分摊位。",
        Jp =
    ...
    [
        new() { En = "搭档 Debuff", Jp = "相方デバフ" },
        new() { En = "优先级顺序", Jp = "優先順位" },
        new() { En = "职责侧", Jp = "ロール側" }
    ];
    ...
    {
        En =
            "每一波选择由哪一组处理塔。默认 AAABBBBA 表示第 1-3 波和第 8 波使用前半组，第 4-7 波使用后半组。塔和万物终结共用一套根据实战形态选择的职责位置表；过去/未来使用相对塔的固定移动。",
        Jp =
    ...
    public override Metadata Metadata => new(13, "汉化版v2");
    ...
    public override void OnSettingsDraw()
    {
        ...
        ImGui.SliderFloat("文本偏移（像素）", ref C.VOffsetMod, -5, 5);
        ImGui.SetNextItemWidth(200f);
        ImGui.SliderFloat("全局覆盖文本缩放倍率", ref C.ScaleMod, 0, 3);
        C.EnsureDefaults();
        ...
        {
            ImGui.Indent();
            DrawDisplayTextSetting("收集分配", ref C.ShowCollectingAssignmentsText, C.CollectingAssignmentsText);
            DrawDisplayTextSetting("等待分配", ref C.ShowWaitingForAssignmentText, C.WaitingForAssignmentText);
            DrawDisplayTextSetting("等待波次", ref C.ShowWaitingForWaveText, C.WaitingForWaveText);
            DrawDisplayTextSetting("当前指令", ref C.ShowActiveInstructionText, C.ActiveInstructionText);
            DrawDisplayTextSetting("无匹配指令", ref C.ShowInactiveInstructionText, C.InactiveInstructionText);
            DrawDisplayTextSetting("目标点覆盖文本", ref C.ShowDestinationOverlayText, C.DestinationOverlayText);
            DrawDisplayTextSetting("头顶分摊 Debuff", ref C.ShowHeadStackDebuffText, C.HeadStackDebuffText);
            DrawDisplayTextSetting("圆形 Debuff", ref C.ShowCircleDebuffText, C.CircleDebuffText);
            DrawDisplayTextSetting("扇形 Debuff", ref C.ShowFanDebuffText, C.FanDebuffText);
            DrawDisplayTextSetting("无 Debuff", ref C.ShowNoDebuffText, C.NoDebuffText);
            DrawDisplayTextSetting("基准塔预览", ref C.ShowReferenceTowerPreviewText, C.ReferenceTowerPreviewText);
            DrawDisplayTextSetting("搭档塔预览", ref C.ShowPairedTowerPreviewText, C.PairedTowerPreviewText);
            ImGui.Unindent();
        }
    ...
    private List<PairValidationLine> ValidatePairSettingsForCurrentParty()
    {
        ...
        {
            lines.Add(PairValidationLine.Error(
                $"只配置了 {configuredCount}/{PairCount} 组搭档。只要配置了任意搭档，4 组搭档都必须有效，否则分配会等待。",
        ...
        {
            lines.Add(PairValidationLine.Warning(
                $"当前小队有 {party.Count} 人。完整搭档检查需要当前 8 人小队或录像小队。",
        ...
        for (var i = 0; i < PairCount; i++)
        {
            ...
            {
                lines.Add(PairValidationLine.Error(
                    $"搭档 {i + 1} 在当前小队中匹配到 {pairPlayers.Count} 人，必须正好匹配 2 人。",
            ...
            foreach (var player in pairPlayers)
            {
                ...
                {
                    lines.Add(PairValidationLine.Error(
                        $"搭档 {i + 1} 与搭档 {previous.PairIndex + 1} 重复：{player.Name}。",
        ...
        {
            lines.Add(PairValidationLine.Error(
                $"显式搭档只覆盖当前小队中 {used.Count}/8 名不重复成员。",
        ...
        bool sortHeadStackByPrevDebuff = false)
    {
        ...
        {
            lines.Add(PairValidationLine.Info(
                "当前看不到 Missing Debuff，因此只检查了静态搭档设置。",
        ...
        {
            lines.Add(PairValidationLine.Warning(
                $"当前只看到 {visibleDebuffs}/8 个 Missing 职责 Debuff。动态分组检查可能不完整。",
        ...
        foreach (var pair in pairs)
        {
            ...
            if (firstIsHead && secondIsHead)
            {
                ...
                {
                    lines.Add(PairValidationLine.Info(
                        $"搭档 {pair.Index + 1} 有 2 名头顶分摊玩家（{pair.First.Name}/{pair.Second.Name}）。“按上一次 Debuff 排序头顶分摊”设置会自动处理左右分配。",
                        $"ペア{pair.Index + 1}は頭割り2人です({pair.First.Name}/{pair.Second.Name})。「頭割りを前回デバフでソート」設定で左右が自動解決されます。"));
                }
                else
                {
                    lines.Add(PairValidationLine.Error(
                        $"搭档 {pair.Index + 1} 当前有 2 名头顶分摊玩家：{pair.First.Name} / {pair.Second.Name}。无法判断前半/后半组分配。",
            ...
            if (firstIsHead || secondIsHead)
            {
                ...
                {
                    lines.Add(PairValidationLine.Warning(
                        $"搭档 {pair.Index + 1} 包含头顶分摊玩家，但搭档当前是 {partnerDebuff}。脚本会等到搭档变为圆形/扇形。",
            ...
            {
                lines.Add(PairValidationLine.Warning(
                    $"搭档 {pair.Index + 1} 不含头顶分摊玩家，当前 Debuff 为 {firstDebuff}/{secondDebuff}。脚本需要两人都是圆形/扇形才能分类该搭档。",
        ...
        {
            lines.Add(PairValidationLine.Warning(
                $"当前快照有 {headCount} 个头顶分摊 Debuff。Forsaken 初始分组通常为 2 个，这可能是后续波次或不完整录像状态。",
        ...
        if (visibleDebuffs == 8 && dynamicErrors == 0)
        {
            lines.Add(PairValidationLine.Info(
                splitHeadPairs
                    ? "当前 Debuff 可按头顶分摊搭档分割规则确定前半/后半组。"
                    : "当前 Debuff 可按整组搭档规则确定前半/后半组。",
                splitHeadPairs
    ...
    private static string PairValidationPrefix(PairValidationSeverity severity)
    {
        return severity switch
        {
            PairValidationSeverity.Error => "[错误]",
            PairValidationSeverity.Warning => "[警告]",
            _ => "[OK]"
        };
    }
    ...
    private void DrawWaveSettings()
    {
        for (var wave = 1; wave <= WaveCount; wave++)
        {
            ...
            if (ImGui.Combo($"{FormatText(C.WaveHeaderText, wave)} 处理组", ref groupIndex, WaveGroupLabels, WaveGroupLabels.Length))
    ...
    private void DrawBasicPositionSettings()
    {
        if (ImGui.TreeNode(C.PastFutureHeaderText.Get()))
        {
            ...
            DrawDisplayTextSetting("文本", ref C.ShowPastFixedText, C.PastFixedText);
            DrawPositionRule(C.PastFixedPosition);
            ImGui.PopID();

            ImGui.PushID("FutureFixed");
            ImGui.TextUnformatted(C.FutureStageLabelText.Get());
            DrawDisplayTextSetting("文本", ref C.ShowFutureFixedText, C.FutureFixedText);
            ...
            DrawDisplayTextSetting("北侧集合文本", ref C.ShowFinalAllThingsEndingGatherNorthText, C.FinalAllThingsEndingGatherNorthText);
            DrawDisplayTextSetting("过去文本", ref C.ShowFinalAllThingsEndingPastText, C.FinalAllThingsEndingPastText);
            DrawDisplayTextSetting("未来文本", ref C.ShowFinalAllThingsEndingFutureText, C.FinalAllThingsEndingFutureText);
            ImGui.PopID();
    ...
    private void DrawPatternPlacementTable(PatternPlacementConfig pattern)
    {
        ...
        ImGui.TableSetupColumn("启用", ImGuiTableColumnFlags.WidthFixed, 42f);
        ImGui.TableSetupColumn("分组", ImGuiTableColumnFlags.WidthFixed, 118f);
        ImGui.TableSetupColumn("Debuff", ImGuiTableColumnFlags.WidthFixed, 96f);
        ImGui.TableSetupColumn("排名", ImGuiTableColumnFlags.WidthFixed, 62f);
        ImGui.TableSetupColumn("文本", ImGuiTableColumnFlags.WidthStretch);
        ImGui.TableSetupColumn("基准", ImGuiTableColumnFlags.WidthFixed, 130f);
        ImGui.TableSetupColumn("角度", ImGuiTableColumnFlags.WidthFixed, 82f);
        ImGui.TableSetupColumn("距离", ImGuiTableColumnFlags.WidthFixed, 82f);
        ImGui.TableSetupColumn("坐标", ImGuiTableColumnFlags.WidthFixed, 145f);
    ...
    private void DrawPositionRule(PositionRule rule)
    {
        ...
        if (ImGui.Combo("基准", ref basisIndex, BasisLabels, BasisLabels.Length))
            rule.Basis = (PositionBasis)Math.Clamp(basisIndex, 0, BasisLabels.Length - 1);

        ImGui.SameLine();
        ImGui.SetNextItemWidth(90f);
        ImGui.DragFloat("角度", ref rule.AngleDeg, 1f, -720f, 720f);

        ImGui.SameLine();
        ImGui.SetNextItemWidth(90f);
        ImGui.DragFloat("距离", ref rule.Range, 0.05f, 0f, 30f);
    }
    ...
    {
        if (!ImGui.CollapsingHeader("调试")) return;

        ImGui.Indent();
        ImGui.TextUnformatted($"激活状态：{_active}");
        ImGui.TextUnformatted($"当前波次：{_currentWave}");
        ImGui.TextUnformatted($"已观测塔波次：{_observedTowerWave}");
        ImGui.TextUnformatted($"当前阶段：{(_hasStage ? _currentStage.ToString() : "无")}");
        ImGui.TextUnformatted($"实战上下文刷新：{_allowLiveContextRefresh}");
        ImGui.TextUnformatted($"基准塔：{FormatMapPosition(_referenceMapPosition)}");
        ImGui.TextUnformatted($"搭档塔：{FormatMapPosition(IsTowerMapPosition(_referenceMapPosition) ? AddMapSteps(_referenceMapPosition, 2) : 0)}");
        ImGui.TextUnformatted($"过去/未来基准：{FormatMapPosition(FixedStageReference())}");
        ImGui.TextUnformatted(_hasPendingTowerDisplay
            ? $"待显示塔：第 {_pendingTowerDisplayWave} 波，基准 {FormatMapPosition(_pendingTowerDisplayReference)}"
            : "待显示塔：无");
        ...
        ImGui.TextUnformatted(TryGetExplicitPairs(priorityParty, out _, out var explicitPairFailure)
            ? "显式搭档：就绪"
            : $"显式搭档：{(HasAnyPairConfiguration() ? explicitPairFailure : "未配置")}");
        ImGui.TextUnformatted($"搭档分割规则：{(C.SplitHeadStackPairs ? "头顶分摊本人进前半组" : "头顶分摊搭档整组进前半组")}");
        ImGui.TextUnformatted($"前半组：{FormatResolvingSet(_firstSetIds)}");
        ImGui.TextUnformatted($"后半组：{FormatResolvingSet(_secondSetIds)}");
        ImGui.TextUnformatted($"初始头顶分摊搭档：{FormatInitialHeadPartnerDebuffs()}");
        ImGui.TextUnformatted($"待生成塔：{FormatMapPositionList(_pendingTowerSpawnPositions)}");
        ImGui.TextUnformatted($"待消除塔：{FormatMapPositionList(_pendingTowerClearPositions)}");
        ImGui.TextUnformatted($"上一形态：H{_lastPattern.Head}/C{_lastPattern.Circle}/F{_lastPattern.Fan}");
        ImGui.TextUnformatted($"上一分组：{_lastSide}");
        ImGui.TextUnformatted($"上一 Debuff/排名：{_lastDebuff} #{_lastDebuffRank}");
        ImGui.TextUnformatted($"上一辅助排名：{_lastSupportRank}");
        ImGui.TextUnformatted($"上一选择器：{(_lastSelectorLabel.Length == 0 ? "无" : _lastSelectorLabel)}");
        ImGui.TextUnformatted($"上一匹配规则：{_lastRuleLabel}");
        ImGui.TextUnformatted($"上下文缓存：{_stageContexts.Count} 名玩家");
        ImGui.TextUnformatted($"第 4 波自标：{(_wave4SelfMarkerHandled ? "已处理" : "等待中")} {(_lastWave4SelfMarkerState.Length == 0 ? "" : _lastWave4SelfMarkerState)}");
        ...
        ImGui.TextUnformatted($"BasePlayer 覆盖：{(string.IsNullOrEmpty(overrideName) ? "无" : overrideName)}");
        ...
        if (me != null)
        {
            ...
            ImGui.TextUnformatted($"BasePlayer 来源：{(local != null && me.AddressEquals(local) ? "LocalPlayer/回退" : "覆盖")}");
            ImGui.TextUnformatted($"BasePlayer 当前 Debuff：{CurrentDebuffFromPlayer(me)}");
            ImGui.TextUnformatted(_stageContexts.TryGetValue(me.EntityId, out var cachedContext)
                ? $"BasePlayer 缓存上下文：{cachedContext.Side} {cachedContext.Debuff} #{cachedContext.DebuffRank} 辅助#{cachedContext.SupportRank}"
                : "BasePlayer 缓存上下文：无");
            ImGui.TextUnformatted($"已有目标点：{_hasDestination}");
            ImGui.TextUnformatted($"目标点：{_myDestination.X:0.00}, {_myDestination.Y:0.00}, {_myDestination.Z:0.00}");
        }

        if (ImGui.TreeNode("预览"))
        {
            ImGui.Checkbox("显示预览", ref C.ShowPreview);

            ImGui.SetNextItemWidth(120f);
            ImGui.DragInt("波次", ref C.PreviewWave, 0.1f, 1, WaveCount);
            C.PreviewWave = Math.Clamp(C.PreviewWave, 1, WaveCount);

            var stageIndex = (int)C.PreviewStage;
            ImGui.SetNextItemWidth(160f);
            if (ImGui.Combo("阶段", ref stageIndex, BuildStageComboLabels(), StageCount))
    ...
    public sealed class RolePlacement
    {
        ...
        public InternationalString Text = new() { En = "职责", Jp = "役割" };
        ...
        public void Ensure()
        {
            ...
            Selector ??= RoleSelector.FromLegacy(Role);
            Selector.Ensure();
            Text ??= new InternationalString { En = "职责", Jp = "役割" };
    ...
    public sealed class Config 
    {
        ...
        private void ApplyChineseLocalization()
        {
            ...
            if (Pairs != null)
            {
                for (var i = 0; i < Pairs.Length; i++)
                {
                    ...
                    Pairs[i].Name = $"Forsaken beta 搭档 {i + 1}";
                    Pairs[i].Description = "显式 Forsaken 搭档。含 1 名头顶分摊的搭档归为前半组；两人都是圆形/扇形的搭档归为后半组。";
                }
            }

            foreach (var placement in BasicStages?
                         .SelectMany(stage => stage.Patterns ?? [])
                         .SelectMany(pattern => pattern.Placements ?? []) ?? [])
            {
                if (placement.Text == null) continue;
                var chineseText = ChinesePlacementText(placement.Text);
                if (string.IsNullOrWhiteSpace(chineseText)) continue;
                SetChineseText(placement.Text, chineseText);
            }
        }

        private static string ChinesePlacementText(InternationalString text)
        {
            var source = !string.IsNullOrWhiteSpace(text.Other) ? text.Other : text.En;
            return source switch
            {
                "Bait fan" => "诱导扇形",
                "Stack support" => "分摊辅助",
                "Debuff stack" => "Debuff 分摊",
                "Debuff stack 1" => "Debuff 分摊 1",
                "Debuff stack 2" => "Debuff 分摊 2",
                "Debuff fan" => "Debuff 扇形",
                "Debuff fan 1" => "Debuff 扇形 1",
                "Debuff fan 2" => "Debuff 扇形 2",
                "Debuff fan 3" => "Debuff 扇形 3",
                "Debuff circle" => "Debuff 圆形",
                "Debuff circle 1" => "Debuff 圆形 1",
                "Debuff circle 2" => "Debuff 圆形 2",
                "Debuff circle 3" => "Debuff 圆形 3",
                "Left stack" => "左分摊",
                "Right stack" => "右分摊",
                "Left cone" => "左扇形",
                "Right cone" => "右扇形",
                "Left cone bait" => "左扇形诱导 1",
                "Right circle" => "右圆形",
                "Left circle" => "左圆形 1",
                "Left TH 1" => "左引导 1",
                "Left TH 2" => "左引导 2",
                "Right D1" => "右引导 1",
                "Right D2" => "右引导 2",
                "Right stack support" => "右分摊外",
                "Left stack support" => "左塔分摊辅助",
                "Head 1 support" => "分摊 1 辅助",
                "Head 2 support" => "分摊 2 辅助",
                "Outer circle" => "外侧圆形",
                "Inner circle" => "内侧圆形",
                "Back fan" => "后侧扇形",
                "Side fan" => "侧边扇形",
                ...
                "Demise NE" => "万物终结 东北",
                "Demise SE" => "万物终结 东南",
                "Head pattern 4-1" => "头顶分摊形态 4-1",
                "Head pattern 4-2" => "头顶分摊形态 4-2",
                "Head pattern 4-3" => "头顶分摊形态 4-3",
                "Head pattern 4-4" => "头顶分摊形态 4-4",
                _ => source
            };
        }

        private static void SetChineseText(InternationalString text, string value)
        {
            text.En = value;
            text.Other = value;
        }
        ...
        {
            En = "过去/未来使用固定移动。第 8 波在过去/未来后北侧集合，万物终结开始时再向北/南移动。",
        ...
        {
            Name = "Forsaken beta 全局优先级",
            Description = "用于优先级搭档 1+3、2+4、5+7、6+8，以及同 Debuff 的动态排名。默认：H2 H1 OT MT M1 M2 R1 R2。",
            PriorityLists =
            [
                new PriorityList
                {
        ...
        public void EnsureDefaults()
        {
            ...
            PastFixedText ??= new InternationalString { En = "塔间", Jp = "塔間" };
            FutureFixedText ??= new InternationalString { En = "对侧", Jp = "反対側" };
            FinalAllThingsEndingGatherNorthText ??= new InternationalString { En = "北侧集合", Jp = "北集合" };
            FinalAllThingsEndingPastText ??= new InternationalString { En = "北", Jp = "北" };
            FinalAllThingsEndingFutureText ??= new InternationalString { En = "南", Jp = "南" };
        ...
        private void MigrateUiTerminology()
        {
            ...
            const string newPriorityDescription =
                "用于优先级搭档 1+3、2+4、5+7、6+8，以及同 Debuff 的动态排名。默认：H2 H1 OT MT M1 M2 R1 R2。";
        ...
        private void EnsurePairSettings()
        {
            ...
            for (var i = 0; i < Pairs.Length; i++)
            {
                ...
                Pairs[i].Name = $"Forsaken beta 搭档 {i + 1}";
                Pairs[i].Description =
                    "显式 Forsaken 搭档。含 1 名头顶分摊的搭档归为前半组；两人都是圆形/扇形的搭档归为后半组。";
            }
        }
        ...
        private void MigrateSupportSelectorDefaults()
        {
            ...
            if (h2c1f1 != null)
            {
                UpdateSupportIfOldDefault(h2c1f1, RoleKind.Support1, PositionBasis.LeftTower, 330f, 4.75f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Fan, 2, "诱导扇形", "扇誘導", PositionBasis.LeftTower, 180f, 4.60f);
                UpdateSupportIfOldDefault(h2c1f1, RoleKind.Support2, PositionBasis.LeftTower, 60f, 4.75f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Fan, 1, "头顶 1 辅助", "頭1補助", PositionBasis.LeftTower, 358f, 4.59f);
                UpdateSupportIfOldDefault(h2c1f1, RoleKind.Support3, PositionBasis.RightTower, 120f, 4.75f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Circle, 1, "头顶 2 辅助", "頭2補助", PositionBasis.RightTower, 9f, 4.71f);
                UpdateSupportIfOldDefault(h2c1f1, RoleKind.Support4, PositionBasis.RightTower, 120f, 4.75f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Circle, 2, "头顶 2 辅助", "頭2補助", PositionBasis.RightTower, 9f, 4.71f);
            }
            ...
            if (h0c2f2 != null)
            {
                UpdateSupportIfOldDefault(h0c2f2, RoleKind.Support1, PositionBasis.ArenaCenter, 0f, 0f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Circle, 1, "外侧圆形", "外側円", PositionBasis.ArenaCenter, 271f, 6.14f);
                UpdateSupportIfOldDefault(h0c2f2, RoleKind.Support2, PositionBasis.ArenaCenter, 0f, 5f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Circle, 2, "内侧圆形", "内側円", PositionBasis.ArenaCenter, 350f, 3.55f);
                UpdateSupportIfOldDefault(h0c2f2, RoleKind.Support3, PositionBasis.ArenaCenter, 45f, 5f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Fan, 1, "后侧扇形", "奥扇", PositionBasis.ArenaCenter, 201f, 4.56f);
                UpdateSupportIfOldDefault(h0c2f2, RoleKind.Support4, PositionBasis.ArenaCenter, 315f, 5f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Fan, 2, "侧边扇形", "横扇", PositionBasis.ArenaCenter, 96f, 3.58f);
            }
        }
        ...
        private void MigrateMirageCoordinateDefaults()
        {
            ...
            if (h2c1f1 != null)
            {
                ...
                UpdateSupportIfOldDefault(h2c1f1, RoleKind.Support1, PositionBasis.LeftTower, 180f, 4.60f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Any, 1, "左塔分摊辅助", "左塔頭補助", PositionBasis.LeftTower, 60f, 4.75f);
                UpdateSupportIfOldDefault(h2c1f1, RoleKind.Support2, PositionBasis.LeftTower, 358f, 4.59f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Any, 2, "诱导扇形", "扇誘導", PositionBasis.LeftTower, 330f, 4.75f);
                UpdateSupportIfOldDefault(h2c1f1, RoleKind.Support3, PositionBasis.RightTower, 9f, 4.71f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Any, 3, "右塔分摊辅助", "右塔頭補助", PositionBasis.RightTower, 270f, 4.75f);
                UpdateSupportIfOldDefault(h2c1f1, RoleKind.Support4, PositionBasis.RightTower, 9f, 4.71f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Any, 4, "右塔分摊辅助", "右塔頭補助", PositionBasis.RightTower, 270f, 4.75f);
            }
            ...
            if (h0c2f2 != null)
            {
                ...
                UpdateSupportIfOldDefault(h0c2f2, RoleKind.Support1, PositionBasis.ArenaCenter, 271f, 6.14f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Any, 1, "万物终结 西北", "消滅NW", PositionBasis.ArenaCenter, 315f, 3.5f);
                UpdateSupportIfOldDefault(h0c2f2, RoleKind.Support2, PositionBasis.ArenaCenter, 350f, 3.55f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Any, 2, "万物终结 西南", "消滅SW", PositionBasis.ArenaCenter, 225f, 3.5f);
                UpdateSupportIfOldDefault(h0c2f2, RoleKind.Support3, PositionBasis.ArenaCenter, 201f, 4.56f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Any, 3, "万物终结 东北", "消滅NE", PositionBasis.ArenaCenter, 45f, 3.55f);
                UpdateSupportIfOldDefault(h0c2f2, RoleKind.Support4, PositionBasis.ArenaCenter, 96f, 3.58f,
                    ParticipantSide.SupportGroup, LiveDebuffKind.Any, 4, "万物终结 东南", "消滅SE", PositionBasis.ArenaCenter, 135f, 3.5f);
            }
    ...
    private static BasicStageConfig CreateDefaultTowerStage(BasicStageKind kind)
    {
        return new BasicStageConfig(
            ...
            new PatternPlacementConfig(
                ...
                Role(RoleKind.Head1, "Debuff 分摊", "デバフ頭割り", PositionBasis.LeftTower, 240f, 3.25f),
                Role(RoleKind.Fan1, "Debuff 扇形 1", "デバフ扇1", PositionBasis.LeftTower, 300f, 3.25f),
                Role(RoleKind.Fan2, "Debuff 扇形 2", "デバフ扇2", PositionBasis.RightTower, 270f, 3.25f),
                Role(RoleKind.Fan3, "Debuff 扇形 3", "デバフ扇3", PositionBasis.RightTower, 300f, 3.25f),
                Role(RoleKind.Support1, "诱导扇形", "扇誘導", PositionBasis.LeftTower, 330f, 4.75f),
                Role(RoleKind.Support2, "分摊辅助", "頭割り補助", PositionBasis.LeftTower, 240f, 4.75f),
                Role(RoleKind.Support3, "分摊辅助", "頭割り補助", PositionBasis.LeftTower, 240f, 4.75f),
                Role(RoleKind.Support4, "分摊辅助", "頭割り補助", PositionBasis.LeftTower, 240f, 4.75f)),
            new PatternPlacementConfig(
                new PatternInfo(1, 3, 0),
                Role(RoleKind.Head1, "Debuff 分摊", "デバフ頭割り", PositionBasis.LeftTower, 240f, 3.25f),
                Role(RoleKind.Circle1, "Debuff 圆形 1", "デバフ円1", PositionBasis.LeftTower, 90f, 3.25f),
                Role(RoleKind.Circle2, "Debuff 圆形 2", "デバフ円2", PositionBasis.RightTower, 210f, 3.25f),
                Role(RoleKind.Circle3, "Debuff 圆形 3", "デバフ円3", PositionBasis.RightTower, 30f, 3.25f),
                Role(RoleKind.Support1, "分摊辅助", "頭割り補助", PositionBasis.LeftTower, 240f, 4.75f),
                Role(RoleKind.Support2, "分摊辅助", "頭割り補助", PositionBasis.LeftTower, 240f, 4.75f),
                Role(RoleKind.Support3, "分摊辅助", "頭割り補助", PositionBasis.LeftTower, 240f, 4.75f),
                Role(RoleKind.Support4, "分摊辅助", "頭割り補助", PositionBasis.LeftTower, 240f, 4.75f)),
            new PatternPlacementConfig(
                new PatternInfo(2, 1, 1),
                Role(RoleKind.Head1, "Debuff 分摊 1", "デバフ頭1", PositionBasis.LeftTower, 60f, 3.25f),
                Role(RoleKind.Head2, "Debuff 分摊 2", "デバフ頭2", PositionBasis.RightTower, 270f, 3.25f),
                Role(RoleKind.Fan1, "Debuff 扇形", "デバフ扇", PositionBasis.LeftTower, 330f, 3.25f),
                Role(RoleKind.Circle1, "Debuff 圆形", "デバフ円", PositionBasis.RightTower, 90f, 3.25f),
                Role(ParticipantSide.SupportGroup, LiveDebuffKind.Any, 1, "左塔分摊辅助", "左塔頭補助", PositionBasis.LeftTower, 60f, 4.75f),
                Role(ParticipantSide.SupportGroup, LiveDebuffKind.Any, 2, "诱导扇形", "扇誘導", PositionBasis.LeftTower, 330f, 4.75f),
                Role(ParticipantSide.SupportGroup, LiveDebuffKind.Any, 3, "右塔分摊辅助", "右塔頭補助", PositionBasis.RightTower, 270f, 4.75f),
                Role(ParticipantSide.SupportGroup, LiveDebuffKind.Any, 4, "右塔分摊辅助", "右塔頭補助", PositionBasis.RightTower, 270f, 4.75f)),
            new PatternPlacementConfig(
                new PatternInfo(0, 2, 2),
                Role(RoleKind.Fan1, "Debuff 扇形 1", "デバフ扇1", PositionBasis.LeftTower, 30f, 3.25f),
                Role(RoleKind.Fan2, "Debuff 扇形 2", "デバフ扇2", PositionBasis.LeftTower, 0f, 3.25f),
                Role(RoleKind.Circle1, "Debuff 圆形 1", "デバフ円1", PositionBasis.RightTower, 90f, 3.25f),
                Role(RoleKind.Circle2, "Debuff 圆形 2", "デバフ円2", PositionBasis.RightTower, 270f, 3.25f),
                Role(ParticipantSide.SupportGroup, LiveDebuffKind.Any, 1, "万物终结 西北", "消滅NW", PositionBasis.ArenaCenter, 315f, 3.5f),
                Role(ParticipantSide.SupportGroup, LiveDebuffKind.Any, 2, "万物终结 西南", "消滅SW", PositionBasis.ArenaCenter, 225f, 3.5f),
                Role(ParticipantSide.SupportGroup, LiveDebuffKind.Any, 3, "万物终结 东北", "消滅NE", PositionBasis.ArenaCenter, 45f, 3.55f),
                Role(ParticipantSide.SupportGroup, LiveDebuffKind.Any, 4, "万物终结 东南", "消滅SE", PositionBasis.ArenaCenter, 135f, 3.5f)),
            new PatternPlacementConfig(
                new PatternInfo(4, 0, 0),
                Role(RoleKind.Head1, "头顶分摊形态 4-1", "頭4処理1", PositionBasis.RightTower, 90f, 3.25f),
                Role(RoleKind.Head2, "头顶分摊形态 4-2", "頭4処理2", PositionBasis.RightTower, 270f, 3.25f),
                Role(RoleKind.Head3, "头顶分摊形态 4-3", "頭4処理3", PositionBasis.LeftTower, 90f, 3.25f),
                Role(RoleKind.Head4, "头顶分摊形态 4-4", "頭4処理4", PositionBasis.LeftTower, 270f, 3.25f),
                Role(RoleKind.Support1, "万物终结 西北", "消滅NW", PositionBasis.ArenaCenter, 315f, 3.5f),
                Role(RoleKind.Support2, "万物终结 西南", "消滅SW", PositionBasis.ArenaCenter, 225f, 3.5f),
                Role(RoleKind.Support3, "万物终结 东北", "消滅NE", PositionBasis.ArenaCenter, 45f, 3.55f),
                Role(RoleKind.Support4, "万物终结 东南", "消滅SE", PositionBasis.ArenaCenter, 135f, 3.5f)));
    }
